...
import numba
numba.config.NUMBA_DEFAULT_NUM_THREADS=6
        ...
        if (i + 1) in cat:
            ...
            for j in range(len(kind)):
                ...
                if sd != 0:
                    ...
                    temp[:, j] = (temp[:, j] - np.mean(temp[:, j])) * rank[i] / len(kind)
        else:
            sd = np.std(col_temp)
            if sd != 0:
...
@jit()
def run_grouping1(data, rank_rate, group: int, times_all: int, cat: List):
    n_sample = int(np.floor(len(data) / group / times_all) * group * times_all)
    datax = data[:n_sample, :]
    print('score')
    deal_data = ZScoreCatCon(datax, cat, rank_rate)
    result, setx = Mdis_Group(deal_data, group, times_all)
    return 1


# @jit()
def run_grouping(data: np.ndarray, rank_rate, group: int, times_all: int, cat: List):
    '''
    # 随机样本量
    alter = 1000 l
    # 特征数
